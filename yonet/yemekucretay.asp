...
'Seçime göre tarihleri veya kiþileri listeleme bölümü 
 if islem="kayitlarilistele" then
	...
	if sicilno="tumu" then
	  ...
	  sorgum="select * from yemek_ucretay where ay="&ay&" and yil="&yil&" and sinif="&sinif&" and sicilno like '%"&sicilno&"%'"&" order by adisoyadi asc"
	end if
  else
...
if kanal="sil" then
 'rs.close
	sorgum="delete * from yemek_ucretay where id="&id
    Rs.Open Sorgum, Baglantim, 1, 3 
	Response.Redirect("giris.asp")
end if	

if Request.Form("guncelle")<>"" then
	rs.open "select * from yemek_ucretay where id="&id,baglantim,1,3
...
<table width="100%" border="0" cellspacing="1" cellpadding="1">
  <tr>
    <td width="300px" valign="top"><!--#include file="yemekucretayust.asp"--></td></tr>
    <tr><td valign="top">
<%
   if islem="kayitlarilistele" then
	if sicilno="tumu" then
...
<%else%>
<!--#include file="yemekucretayalt.asp"-->
<%end if%>
</td>
  </tr>
</table>
