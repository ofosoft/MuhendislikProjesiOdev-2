...
<%  
 if Request.Form("album_guncelle")<>"" then
 	 sorgum="select * from video_kat where id="&id
 ...
 if id<>"" then
 	sorgum="select * from video_kat where id="&id&" order by id desc"
 else
 	sorgum="select * from video_kat order by id desc"
 end if
 Rs.Open Sorgum, Baglantim, 1, 3 
 if rs.eof or rs.bof then%>

    	<div align="center"><a href="?kanal=ekle">Kategori Bulunamadi. Eklemek Için Tiklayin. </a>
    	    <% if kanal="ekle" then
			rs.addnew
...
<table width="100%" border="0" cellpadding="3" cellspacing="3" >
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="30%">Toplam<strong> <%=rs.recordcount %> </strong>Adet Kayit Bulunmaktadir. </td>
...
if kanal="sil" then %>
<% rs.close
	sorgum="delete * from video_kat where id="&id
    Rs.Open Sorgum, Baglantim, 1, 3 
	Response.Redirect("giris.asp")
	 %>
