...
<%if session("yetkisi")=1 or session("yetkisi")=6 or session("yetkisi")=7  then%>
	...
	elseif islem="guncelle" then
		if Request.Form("guncelle")<>"" then
			...
			if Request.Form("proje")<>"" then
				...
				session("projeid")=Request.Form("proje")
			end if
			ers.update
			ers.close
			if aranan<>"" then
	...
	<script type="text/javascript">
		...
		<%if ilid>0 and ilceid>0 and koyid>0 then%>
			&nbsp;<a href="?islem=arsivekle&ilid=<%= ilid %>&ilceid=<%=ilceid%>&koyid=<%=koyid%>" class="buton" style="color:#f8f8f8">Yeni Arþiv Ekle</a><br><br>
			<% 
			if islem="ozellistele" and (aranan<>"" or arananada<>"" or arananparsel<>"") then
				...
				ers.open "select * from arsiv where ilkod="&ilid&" and ilcekod="&ilceid&" and koykod="&koyid&" "&ada&" "&parsel&" and (adsoy like '%"&aranan&"%'"&" or adsoy like '%"&bkelime&"%'"&" or adsoy like '%"&kkelime&"%'"&") order by kayitno desc",emlak,1,3
			else
			...
			<tr>
			  <td id="altzemin" colspan="8" align="center">
				<form name="form1" method="post" action="?islem=ozellistele&ilid=<%= ilid %>&ilceid=<%=ilceid%>&koyid=<%=koyid%>">
					&nbsp;&nbsp;<b id="menubg">Seçime Göre Arama Yap »</b>&nbsp;
					<input name="aranan" type="text" id="aranan" placeholder="* Aranacak Kiþi Adý veya Soyadý" value="<%=aranan%>" size="35" style="width:250px; font-size:13px; border-radius:5px; padding:4px;">&nbsp;
			...
			<form name="form1" method="post" action="?islem=guncelle&id=<%= ers("id") %>&ilid=<%= ilid %>&ilceid=<%=ilceid%>&koyid=<%=koyid%>&shf=<%=shf%>&aranan=<%=aranan%>&arananada=<%=arananada%>&arananparsel=<%=arananparsel%>">
				<tr class="tablokenar">
					<td><input name="id" type="text" id="id" value="<%= ers("kayitno") %>" size="3" disabled="disabled" style="padding-left:6px"></td>
					<td align="center"><input name="adi" type="text" id="adi" value="<%= ers("adsoy") %>" size="33"></td>
					<td align="center"><input name="ada" type="text" id="ada" value="<%= ers("ada") %>" size="2"></td>
					<td><input name="parsel" type="text" id="parsel" value="<%= ers("parsel") %>" size="3"></td>
					<td>
					<%if ers("adsoy")<>"" then%>
						<input name="projebos" type="text" id="projebos" value="<%= emlakprojebul(ers("proje")) %>" size="30" disabled="disabled">
					<%else%>
					<%if session("projeid")<>"" then
					pid=int(session("projeid"))
					else
					pid=0
					end if%>						
						<select name="proje" id="proje" style="width:230px; font-size:13px; border-radius:5px; padding:4px;">
							<%
							esorgum="Select * from projeler order by projead asc"
							erz.Open esorgum, emlak, 1, 3%>
							<% do while not erz.eof
							if erz.eof then exit do%>
								<option value="<%= erz("recid") %>" <%if pid=int(erz("recid")) then%> selected="selected"<%end if%>><%= erz("projead") %></option>
							<%erz.movenext
							loop
							erz.close %>		
						</select>
			...
			<tr>
				<td height="40" colspan="8">
				 <%for y=1 to sayfa 
					...
					<% else %>
					 <a href="?sayfa=emlak arsiv&shf=<%= y %>&ilid=<%= ilid %>&ilceid=<%=ilceid%>&koyid=<%=koyid%>&islem=<%=islem%>&aranan=<%=aranan%>&arananada=<%=arananada%>&arananparsel=<%=arananparsel%>" class="buton" style="color:#f8f8f8; width:40px"><%= y %></a>	
					<% end if
				next %>
