...
arrBaseFolder(0)="/resimler/urun_altkat/"'Use "Relative to Root" Path
arrBaseName(0)="URUN ALT KATEGORI RESIMLERI"
%>
...
<base target="_self">
<html>
<title>Resim Y&uuml;kle</title><head>
...
<%
	...
	sorgum="Select * from urun_altkat where id="&Request.QueryString("id") 
	Rf.Open Sorgum, Baglantim, 1, 3  
		rf(""&yeri&"")=Request.Form("inpSource")
		rf.update
		rf.close 

Response.Write("Güncelleme Yapýldý.")
end if
%>
...
<td class="dialogFooter" style="height:40px;padding-right:15px;" align=right valign=middle>
	...
	<tr>
	<td>
	<form name="form1" method="post" action="?id=<%=Request.QueryString("id") %>&yer=<%=yeri%>&islem=Guncelle">
