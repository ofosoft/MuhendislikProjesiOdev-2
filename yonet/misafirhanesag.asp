<table width="100%" border="0" cellpadding="3" cellspacing="0" >
  <tr>
    ...
    <td class="altcizgiduz"><div align="right" style="font-weight: bold">Yemek Ücret</div></td>
  ...
  <% if rs.eof or rs.bof then
    ...
    <tr class="altkesikcizgi">
     ...
     <td width="220" align="right" class="altcizgikesik">
	 <%yazi= rs("ucret")
	sonuc = InStr(1,yazi, ",")
	if sonuc<>0 then
  ...
  <a href="?kanal=sil&amp;id=<%= rs("id") %>&sicilno=<%= rs("sicilno") %>&ucret=<%= rs("ucret") %>&ay=<%=right(left(rs("tarih"),5),3) %>" onclick="return confirm('Kaydý Silmek Ýstediginizden Emin Misiniz?');"><img src="images/sil.jpg"border="0" /></a></div></td>
  </tr>
	 <% 
      rs.movenext
	next%>
  ...
  <tr>
    ...
    Toplam<b> <%=rs.recordcount %> </b> Kiþiye Yemek Kaydedilmiþtir.</td>
