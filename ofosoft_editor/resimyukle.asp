<% if Session("adminmi")<>"evet" then%><!--#include file="sifre.asp"--><%end if %>
<%
yeri=Request.QueryString("yer")
kyer=request.QueryString("klasor")
kid=Request.QueryString("id")
response.Expires=0
kyerk=kyer
...
if kyerk="haber" and kid>42 then
arrBaseFolder(0)="/resimler/"&kyerk&"/"&kid&"/"	'Use "Relative to Root" Path
else
arrBaseFolder(0)="/resimler/"&kyerk&"/"'Use "Relative to Root" Path
end if
arrBaseName(0)=""&ucase(kyerk)&" RESIMLERI"
...
<base target="_self">
<html>
<title><%=ucase(kyerk)%> Resmi Yükle</title><head>
...
function getAction()
	...
	if(sQueryString.indexOf("=")==-1) 
		{//no querystring
		sAction="?ffilter="+document.getElementById("selFilter").value;
		}
	else 
		{
		sAction="?"+sQueryString+"&ffilter="+document.getElementById("selFilter").value
		}
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	return sAction;
	}
...
if Request.QueryString("islem")="Guncelle" and Request.QueryString("id")<>"" then%>
<!--#include file="../ayarlar.asp"-->
<%
	vt_yol ="../" & vtyolu & "/"& vtadi &""
	set Baglantim = server.createobject("adodb.connection")
	Baglantim.open "provider=microsoft.jet.oledb.4.0;data source=" & Server.mappath(vt_yol & ".mdb")

	Set Rf=Server.CreateObject("Adodb.Recordset")
	if kyer="haber" then
	sorgum="Select * from "&kyer&" where hkodu="&Request.QueryString("id") 
	else
	sorgum="Select * from "&kyer&" where id="&Request.QueryString("id") 
	end if
	Rf.Open Sorgum, Baglantim, 1, 3  
		rf(""&yeri&"")=Request.Form("inpSource")
		rf.update
		rf.close%> 
<table width="100%" border="0" cellspacing="2" cellpadding="2" height="25" bgcolor="#000099">
  <tr>
    <td align="center"><FONT color="#FFFFFF"><strong>RESIM EKLEME ISLEMI BASARIYLA TAMAMLANMISTIR.</strong></FONT></td>
  </tr>
</table><br>
<center><input  type="submit" value="Pencereyi Kapat" onClick="javascript:self.close();" onKeyPress="javascript:self.close();"></center><br>
<%if 1=2 then%>
...
<td valign=top style="background:url('bg.gif') no-repeat right bottom;padding-top:5px;padding-left:5px;padding-right:5px;padding-bottom:0px;">
		...
		<td>
				...
				<td nowrap>
					<span onClick="newFolder()" style="cursor:pointer;" <%=sFolderAdmin%>><u><span name="txtLang" id="txtLang">Yeni Klas&ouml;r </span></u></span>&nbsp;
					<span onClick="deleteFolder()" style="cursor:pointer;" <%=sFolderAdmin%>><u><span name="txtLang" id="txtLang">Klas&ouml;r Sil </span></u></span>
				</td>
		...
		<td valign=top align="center">
				...
				<tr>
				<td>
					<div id="idPreview" style="text-align:center;overflow:auto;width:450;height:400;border:#d7d7d7 5px solid;border-bottom:#d7d7d7 3px solid;background:#ffffff;margin-right:2;"></div>
...
<td class="dialogFooter" style="height:40px;padding-right:15px;" align=right valign=middle>
	...
	<tr>
	<td>
	<form name="form1" method="post" action="?id=<%=Request.QueryString("id") %>&yer=<%=yeri%>&klasor=<%=kyer%>&islem=Guncelle">
	<input type="text" id="inpSource" name="inpSource" style="border:#cfcfcf 1px solid;width:295" class="inpTxt" >
	<input name="btnOk" id="btnOk" type="submit" value=" ok "  class="inpBtn" onMouseOver="this.className='inpBtnOver';" onMouseOut="this.className='inpBtnOut'">
